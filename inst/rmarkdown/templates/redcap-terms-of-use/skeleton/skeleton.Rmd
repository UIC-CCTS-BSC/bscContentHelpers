...
: A secure, web-based application for building and managing online surveys and databases. REDCap was created by [Vanderbilt University](https://projectredcap.org/) in 2004.

CCTS

: UIC's [Center for Clinical and Translational Science](https://ccts.uic.edu/). CCTS administers the UIC-specific installation of REDCap and provides training and consultation services for UIC health researchers.

IHRP

: UIC's [Institute for Health Research and Policy](https://www.ihrp.uic.edu/). IHRP manages the REDCap server and IT infrastructure at UIC.
...
: A full-access user who has requested the creation of accounts for one or more access-only users. All non-UIC users must have a sponsor affiliated with UIC health research.
...
# User Accounts
## New Accounts
1. Any UIC-affiliated health researcher who needs to create or copy REDCap projects can request a full-access user account. To request an account, the user must submit a formal service request through the [CCTS workflow system](https://webapps.ccts.uic.edu/CCTS_Service_Requests/).
1. Any individual working on a UIC REDCap project can obtain an access-only user account, provided he or she is sponsored by a full-access user. Either the access-only user or the sponsor can request the account by emailing the [REDCap admin team](redcapihrp@uic.edu). Requests must include the access-only user's name and email address and the sponsor's name and email address or REDCap username.
...
1. Users may request password resets for their own accounts by emailing the [REDCap admin team](redcapihrp@uic.edu). A user may not initiate a password reset on behalf of another user.
...
1. Each individual who accesses a REDCap project must have a unique username and password. Users must keep their login credentials private and must not share these credentials with any other person.
...
1. Users with no activity for at least one year will have their accounts suspended. A suspended user can email the [REDCap admin team](redcapihrp@uic.edu) to request account reactivation.
1. Access-only user accounts have a default expiration date of one year from creation. A user's sponsor can manage expiration dates via the sponsor dashboard.
1. Users may request deletion of their own accounts from the REDCap system by emailing the [REDCap admin team](redcapihrp@uic.edu). Account deletion may be initiated by a third party, but no account will be deleted unless the account owner confirms the request.
...
1. The REDCap admin team creates and maintains user accounts at no charge and provides training courses, general instructions, and troubleshooting to all UIC health researchers and affiliated users by [email](redcapihrp@uic.edu). Users can also request an in-person REDCap consultation meeting through the [CCTS workflow system](https://webapps.ccts.uic.edu/CCTS_Service_Requests/).
1. Email support is available Monday through Friday, 9 a.m. to 5 p.m. Limited email support may be available during campus holidays and staff absences. Users will be notified a week in advance of periods of planned support downtime.
1. The REDCap admin team will respond to most emails within two business days. In times of high volume, requests may take longer to complete.
...
<!-- 1. Security specs implemented by CCTS REDCap at UIC -->
