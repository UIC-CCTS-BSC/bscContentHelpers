...
This page includes publications and tools that our consultants have found useful`r if(!is.null(rmarkdown::metadata$useful_for)) {glue::glue(" {rmarkdown::metadata$useful_for}")}`. `r if(knitr::is_html_output()) {"The resource list can be downloaded in PDF and [BibTeX](https://www.bibtex.org/) format at the [bottom of this page](#attachments)."}` For more information on this topic, including advice about how to apply it in your research, consider [scheduling a consultation with a biostatistician](https://ccts.uic.edu/services/requesting-services/).
...
```{r}
show_bib <- function(name) {
	x = RefManageR::ReadBib(
		glue::glue("files/{name}.bib")
		)
