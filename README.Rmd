...
The document development and rendering process relies on three files: [an .Rmd template](#an-r-markdown-template), [an output format](#an-output-format), and [a knit function](#a-knit-function).

### An R Markdown Template

At its most basic, an .Rmd template is a sample file, written in R Markdown, that may include standard headers, sample code, or boilerplate text. An Rmd template defines the content and structure of a document. 
...
Like all R Markdown documents, templates start with a section called a YAML header, bordered by three tick marks (`---`). This section defines the settings of your final document, including metadata (e.g., title, author, date) and output format and behavior (e.g., Word document with a certain set of fonts, saved with a specific file name in a specific location on your computer. Custom templates in this package have some default settings, so you can probably edit your document's title, author, and date information and leave the rest of the settings alone.

The rest of the document is written in R Markdown. Study the boilerplate text that each template contains; it will give you pointers about how to craft your own document in R Markdown. Delete the boilerplate text and add your own.


### An Output Format

Once the content has been developed, one or more output formats can be applied. In this context, an "output format" is an R function that defines the file type (e.g., docx or pptx) and the look and feel (e.g., colors, fonts) of your final document. 

Note that multiple different formats can be applied to the same template. (This is one of the benefits of using R Markdown!) For example, you might have an .Rmd report that can be knit as an html draft during content development. The final version could be knit to a pdf with custom headers, fonts, and colors. Each format has default settings, some of which can be customized at the document level.

You will rarely, if ever, execute the function directly. Instead, include the format namein your draft's YAML header. To use default settings, just include the name:

```
output: word_document
```

To declare custom settings ("options"), include those in the header too, indented by two spaces. For example, to include a table of contents, you might include:
...
A knit function is an R function that controls the behavior of the knit button; in other words, it determines what steps get taken, and in what order, to turn the Rmd draft into the pdf (or html or docx or pptx) document. Like with an output format, you don't usually execute a knit function directly. Instead, you include it in a document's YAML header:

`knit: function_name`

It gets executed when you knit the document by clicking the `knit` button or typing `Ctrl + Shift + k`.

Currently, this package includes one knit function (`bsc_knit`). This knit function is declared in all package templates. It allows you to knit multiple formats at once with some custom options (see below). If you would prefer to use [R Markdown's default knitting behavior](https://bookdown.org/yihui/rmarkdown/compile.html), simply delete this YAML option.
