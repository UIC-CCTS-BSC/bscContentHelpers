...
Step 0 [Recommended]: Sync the remote repository (i.e., the current version of the files on GitHub) with your local copy. Then create a new development branch.
...
Put supporting documents (e.g., image files or document-specific stylesheets that should be copied every time the template is used) in the `/skeleton` subfolder. (Note that you don't need to include files that are part of the output format. Those should be in `inst/rmd_files` and referred to in R scripts by `system.file("rmd_files/filename.ext", package = "bscContentHelpers")`.)
